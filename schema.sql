...
CREATE TABLE Vehicle (
    ...
    location ENUM('main_lot', 'remote_lot', 'other_dealership', 'in_trasnsit')
);
...
CREATE TABLE Customer (
    ...
    phone VARCHAR(20),
    email VARCHAR(255),
    customer_type ENUM('purhcase', 'service', 'visit')
);
...
CREATE TABLE Sale (
    ...
    salesperson_id INT,
    FOREIGN KEY (customer_id) REFERENCES Customer(customer_id),
    FOREIGN KEY (vehicle_id) REFERNCES Vehicle(vehicle_id),
    FOREIGN KEY (salesperson_id) REFERNCES Employee(employee_id)
);

CREATE TABLE Service (
    serivice_id INT AUTO_INCREMENT PRIMARY KEY,
    arrival_mileage INT,
